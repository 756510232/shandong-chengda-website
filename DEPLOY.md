...
- 客服电话 131 8887 8975、邮箱 clrzzl@163.com、ICP 备案号已替换为实际信息
- 地图区域为装饰性占位 → 可接入高德/百度地图 API
- 联系表单采用「暖白卡片容器内嵌 WPS 表单」方案（见 `contact.html` 的 `.form-embed-wrap`）→ WPS 表单直接内嵌于与全站一致的白色卡片中，视觉融入周边环境，数据自动进入 WPS 表格后台，无需自建接口
...
联系我们页的留资表单采用「**暖白卡片容器内嵌 WPS 表单**」方案：
外层标题、说明、边框均为网站视觉（暖白+橙红、白色卡片），内部直接嵌入 WPS 表单 iframe，访客在页面上即可填写，数据自动进入 WPS 表格后台。

- 容器结构：`contact.html` 中 `.form-embed-wrap`（`.wps-form-embed` 内为 WPS iframe，常驻加载）
- 当前嵌入表单：`https://f.kdocs.cn/g/QrfVAqFo/`（免费评估额度收集表）
...
- 兼容处理：`js/main.js` 已重构，不再包含旧表单提交逻辑；后续改为自建表单时再接入后端接口
...
### 隐私清理（按需求）
- 后期根据业务需要，网站已恢复客服电话 131 8887 8975；总部座机与详细地址仍不展示。
