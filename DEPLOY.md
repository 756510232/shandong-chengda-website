...
- 联系表单已用 **WPS 表单（金山文档）** 通过 iframe 嵌入（见 `contact.html` 的 `.wps-form-embed`）→ 访客在站内直接填写，数据自动进入你的 WPS 表格后台，无需自建接口
...
联系我们页的留资表单已通过 **WPS 表单 / 金山文档** 的 iframe 直接嵌入（`contact.html` 中的 `.wps-form-embed` 区块）。
访客在网站内即可填写，提交的数据自动进入你的 WPS 表格后台，可随时登录 WPS 查看、导出、设提醒。

- 当前嵌入表单：`https://f.kdocs.cn/g/axaFE7mS/`（网站留资-收集表）
- 后台查看：登录 WPS / 金山文档 → 打开该收集表 → 查看提交记录与统计
- 更换表单：若新建 / 替换收集表，只需改 `contact.html` 里 iframe 的 `src` 地址，重新 `git push` 即可
